...
def add_book(author, title, pages, publisher, year):
    ...
    authors = session.query(Authors)                        .filter(Authors.id == author)                        .first()
...
def filter_4(N):
    authors = session.query(Authors,  func.count(Books.id))         .join(Books).group_by(Authors.id)         .having(func.count(Books.id) > N).all()
